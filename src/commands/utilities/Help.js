	...
	async run(message, [command]) {
		...
		if (command) {
			...
			if (!this.client.owners.includes(message.author.id)) {
				categories = this.client.utils.removeDupelicates(this.client.commands.filter(cmd => cmd.category !== 'Owner').map(cmd => cmd.category));
			} else {
				categories = this.client.utils.removeDupelicates(this.client.commands.map(cmd => cmd.category));
			}
